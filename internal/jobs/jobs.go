// Package jobs tracks in-flight PDF operations so the HTTP layer can stream
// their progress over SSE and support cancellation. State is in-memory
// only — this is a disposable local tool, not a service.
package jobs
...
// Event is one step of a job's progress, matching the terminal fields
// (Path/Filename on success, Code on failure).
type Event struct {
	Stage    string  `json:"stage"` // processing, done, error, canceled
